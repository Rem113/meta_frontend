...
const CreateImage: React.FC = () => {
    ...
    const validate = (
        ...
        if (version.trim().length === 0) errors.version = "Please enter a version number"
    ...
    const submit = async () => {
        ...
        if (Object.keys(errors).length > 0) setErrors(errors)
            ...
            try {
                ...
                toast(() => "Failed to create an image: " + error, {theme: "dark", type: "error"})
            }
        }
    }
    ...
    return <>
        ...
        <form onSubmit={submit}>
            <TextInput value={name} onChange={change(setName)} label={'Name'} error={errors.name}/>
            <TextInput
                value={version}
            ...
            />
            <FileInput label={"Image"} file={file} onFileSelected={setFile} error={errors.file}/>
            <RaisedButton text={"Add image"} icon={add} alt={"add image"} onClick={submit} disabled={isCreatingImage}/>
        </form>
    </>
}
