	...
	useEffect(() => {
		if (execution !== undefined) {
			...
			const passedStatuses = execution.events
				.filter(event => event.type === ScenarioPlayingEventType.STEP_PASSED)
				.reduce(
					(accumulator, event) => ({
						...accumulator,
			...
			setStepStatus({ ...stepStatus, ...failedStatuses, ...passedStatuses })
			...
			const stepMessages = execution.events
				...
				.map(event => {
					if (event.type === ScenarioPlayingEventType.STEP_FAILED) {
						...
						return { message: stepFailed.message, step: stepFailed.step }
					} else {
						const stepPassed = event as StepPassed
						return { message: stepPassed.message, step: stepPassed.step }
					}
				})
				.reduce(
					(accumulator, current) => ({
			...
			const logs = execution.events
				.filter(event => event.type === ScenarioPlayingEventType.LOG_RECEIVED)
